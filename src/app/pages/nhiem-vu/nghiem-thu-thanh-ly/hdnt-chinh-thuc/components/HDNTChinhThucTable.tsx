...
import { IResult, IHoiDongNghiemThu } from '@/models';
import { TDTable } from '@/app/components';
import { useHDNTChinhThucTable } from './useHDNTChinhThucTable';
import { HDNTChinhThucDetailModal } from './HDNTChinhThucDetailModal';
import { toViewDateString } from '@/utils/utils';

interface HDNTChinhThucTableProps {
  searchData?: SearchData;
}

export const HDNTChinhThucTable: React.FC<HDNTChinhThucTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const modalVisibleCapMot = useSelector((state: RootState) => state.modal.modalVisibleCapMot);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useHDNTChinhThucTable({ searchData });

  const handleAction = useCallback(
    async (type: string, record: IHoiDongNghiemThu): Promise<void> => {
      try {
        switch (type) {
          case 'ket-qua':
          ...
          case 'detail':
            ...
            break;
          case 'delete':
            const response = await requestDELETE<IResult<boolean>>(`HoiDongNghiemThus/${record.id}`);
  ...
  const columns: TableProps<IHoiDongNghiemThu>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
  ...
  return (
    <>
      ...
      {modalVisible ? <HDNTChinhThucDetailModal totalCount={totalCount} /> : <></>}
    </>
  );
};
