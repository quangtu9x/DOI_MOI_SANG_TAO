...
import { Form, Input, Spin, TreeSelect, Steps, Radio } from 'antd';
...
import { Gender, IOrganizationUnit, IPaginationResponse, IPosition, IResult } from '@/models';
...
export const UserDetailModal = (props: any) => {
  ...
  const accountTypeCardMeta: Record<UserType, { label: string; description: string; icon: string, note: string }> = {
    ...
    [UserType.Specialist]: {
      ...
      note: 'Tên tài khoản phải trùng với mã số viên chức'
    },
    [UserType.FromPortal]: {
  ...
  const treeData = useMemo(() => {
    if (accountType === null || accountType === undefined || organizationUnits.length === 0) {
      return [];
    }
    ...
    const buildTreeNode = (items: IOrganizationUnit[], parentId: string | number | null = null): any => {
      return items
        .filter(item => String(item.parentId ?? '__root__') === String(parentId ?? '__root__'))
        .map(item => ({
          title: item.name,
          value: item.id,
          key: item.id,
          children: buildTreeNode(childrenMap[item.id] || [], item.id),
        }));
    };

    return buildTreeNode(organizationUnits, null);
  }, [organizationUnits, accountType]);
                          ...
                          <TDSelect
                            ...
                            onChange={(value, current: any) => {
                              if (value && current) {
                                ...
                                const hasExistingUser = current.userId && current.userName;
                                if (hasExistingUser) {
