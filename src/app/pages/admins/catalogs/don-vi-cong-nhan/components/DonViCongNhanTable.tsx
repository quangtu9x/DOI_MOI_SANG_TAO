...
import { DonViCongNhanDetailModal } from './DonViCongNhanDetailModal';
import { useDonViCongNhanTable } from './useDonViCongNhanTable';

interface CategoryTableProps {
  searchData?: SearchData;
}

export const DonViCongNhanTable: React.FC<CategoryTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize, categoryGroupId } = useDonViCongNhanTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <DonViCongNhanDetailModal totalCount={totalCount} categoryGroupId={categoryGroupId} /> : <></>}
    </>
  );
};
