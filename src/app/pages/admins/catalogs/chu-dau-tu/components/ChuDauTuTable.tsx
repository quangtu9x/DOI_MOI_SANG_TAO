...
import { ChuDauTuDetailModal } from './ChuDauTuDetailModal';
import { useChuDauTuTable } from './useChuDauTuTable';

interface CategoryTableProps {
  searchData?: SearchData;
}

export const ChuDauTuTable: React.FC<CategoryTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize, categoryGroupId } = useChuDauTuTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <ChuDauTuDetailModal totalCount={totalCount} categoryGroupId={categoryGroupId} /> : <></>}
    </>
  );
};
