...
import { LoaiSangKienDetailModal } from './LoaiSangKienDetailModal';
import { useLoaiSangKienTable } from './useLoaiSangKienTable';

interface CategoryTableProps {
  searchData?: SearchData;
}

export const LoaiSangKienTable: React.FC<CategoryTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize, categoryGroupId } = useLoaiSangKienTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <LoaiSangKienDetailModal totalCount={totalCount} categoryGroupId={categoryGroupId} /> : <></>}
    </>
  );
};
