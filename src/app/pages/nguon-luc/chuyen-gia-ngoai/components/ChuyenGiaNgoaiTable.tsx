...
import { IResult, IChuyenGia } from '@/models';
import { TDTable } from '@/app/components';
import { useChuyenGiaNgoaiTable } from './useChuyenGiaNgoaiTable';
import { ChuyenGiaNgoaiDetailModal } from './ChuyenGiaNgoaiDetailModal';
import { GENDERS } from '@/data';
import { formatName, getThumbnailUrl, toViewDate, toViewDateString } from '@/utils/utils';

interface DataTableProps {
  searchData?: SearchData;
}

export const ChuyenGiaNgoaiTable: React.FC<DataTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useChuyenGiaNgoaiTable({ searchData });

  const handleAction = useCallback(
    async (type: string, record: IChuyenGia): Promise<void> => {
      try {
        switch (type) {
          case 'detail':
            ...
            break;
          case 'delete':
            const response = await requestDELETE<IResult<boolean>>(`ChuyenGias/${record.id}`);
  ...
  const columns: TableProps<IChuyenGia>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
  ...
  return (
    <>
      ...
      {modalVisible ? <ChuyenGiaNgoaiDetailModal /> : <></>}
    </>
  );
};
