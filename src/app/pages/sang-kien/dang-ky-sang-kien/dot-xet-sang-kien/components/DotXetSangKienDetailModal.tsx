...
import { IDotXetSangKien, IPaginationResponse, IResult } from '@/models';
...
export const DotXetSangKienDetailModal = props => {
  const dispatch: AppDispatch = useDispatch();
  const { totalCount } = props
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IDotXetSangKien | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IDotXetSangKien>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IDotXetSangKien>>(`DotXetSangKiens/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const formData: IDotXetSangKien = {
        ...values,
        ...(id && { id }),
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`DotXetSangKiens/${id}`, formData)
        : await requestPOST<IResult<string>>(`DotXetSangKiens`, formData);
