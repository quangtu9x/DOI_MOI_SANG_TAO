...
const getLoginLogTypeLabel = (type?: string) => (type === 'Logout' ? 'Đăng xuất' : 'Đăng nhập');
    ...
    try {
      ...
      if (allLogs.length > 0) {
        ...
        const rows = allLogs.map((log, index) => ({
          ...
          'Loại thao tác': getLoginLogTypeLabel(log.type),
          'Địa chỉ IP': log.ip || '',
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Nhật ký đăng nhập');

        // Generate filename
        const timestamp = dayjs().format('YYYYMMDD_HHmmss');
        const filename = `Nhat_ky_dang_nhap_${timestamp}.xlsx`;
    ...
    try {
      ...
      if (allLogs.length > 0) {
          ...
          <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
            <thead>
              <tr>
                ...
                <th style="border: 1px solid #ddd; padding: 6px; background-color: #f2f2f2; font-weight: bold; text-align: center; width: 10%;">Loại thao tác</th>
        ...
        allLogs.forEach((log, index) => {
          ...
          tableHTML += `<td style="border: 1px solid #ddd; padding: 4px; text-align: center;">${getLoginLogTypeLabel(log.type)}</td>`;
        ...
        try {
          ...
          // Generate filename
          const timestamp = dayjs().format('YYYYMMDD_HHmmss');
          const filename = `Nhat_ky_dang_nhap_${timestamp}.pdf`;
