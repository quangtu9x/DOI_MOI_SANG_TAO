...
import { IPaginationResponse, ILoginLog } from '@/models';
...
export const useLoginLogTable = ({ searchData, initialPageSize = 50 }: LoginLogProps) => {
  const dispatch: AppDispatch = useDispatch();
  const random = useSelector((state: RootState) => state.global.random);

  const [data, setData] = useState<ILoginLog[]>([]);
