...
import { useKQDGChinhThucTable } from './useKQDGChinhThucTable';
import { KQDGChinhThucDetailModal } from './KQDGChinhThucDetailModal';
import { toViewDateString } from '@/utils/utils';
import { XEP_LOAI_NGHIEM_THU } from '@/data';

interface KQDGChinhThucTableProps {
  searchData?: SearchData;
}

export const KQDGChinhThucTable: React.FC<KQDGChinhThucTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const modalVisibleCapMot = useSelector((state: RootState) => state.modal.modalVisibleCapMot);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useKQDGChinhThucTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <KQDGChinhThucDetailModal totalCount={totalCount} /> : <></>}
    </>
  );
};
