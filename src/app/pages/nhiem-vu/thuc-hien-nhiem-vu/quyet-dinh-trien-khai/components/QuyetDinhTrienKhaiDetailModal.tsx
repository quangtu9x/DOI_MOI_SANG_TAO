...
import { IBienBanMoHoSo, IChiTietMoHoSo, IPaginationResponse, IQuyetDinhTrienKhai, IResult, LoaiChiTietMoHoSo } from '@/models';
...
export const QuyetDinhTrienKhaiDetailModal = props => {
  const dispatch: AppDispatch = useDispatch();
  const { totalCount } = props
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IQuyetDinhTrienKhai | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IQuyetDinhTrienKhai>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IQuyetDinhTrienKhai>>(`QuyetDinhTrienKhais/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const formData: IQuyetDinhTrienKhai = {
        ...values,
        ...(id && { id }),
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`QuyetDinhTrienKhais/${id}`, formData)
        : await requestPOST<IResult<string>>(`QuyetDinhTrienKhais`, formData);
          ...
          {!isLoading && (
            <Form<IQuyetDinhTrienKhai>
              ...
              <div className="row">
                ...
                <div className="col-xl-8 col-lg-8">
                  <Form.Item label="Tên quyết định" name="ten" rules={[{ required: true, message: 'Không được để trống!' }]}>
                    <Input placeholder="" />
                  </Form.Item>
                </div>
                <div className="col-xl-4 col-lg-4">
                ...
                <div className="col-xl-4 col-lg-4">
                  <Form.Item label="Kinh phí phê duyệt" name="kinhPhiPheDuyet"
                    rules={[
