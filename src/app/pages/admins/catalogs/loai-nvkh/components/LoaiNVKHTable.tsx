...
import { LoaiNVKHDetailModal } from './LoaiNVKHDetailModal';
import { useLoaiNVKHTable } from './useLoaiNVKHTable';

interface CategoryTableProps {
  searchData?: SearchData;
}

export const LoaiNVKHTable: React.FC<CategoryTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize, categoryGroupId } = useLoaiNVKHTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <LoaiNVKHDetailModal totalCount={totalCount} categoryGroupId={categoryGroupId} /> : <></>}
    </>
  );
};
