...
import { Form, Input, Select, Spin, Steps, InputNumber } from 'antd';
import { Modal, Button } from 'react-bootstrap';

import * as actionsModal from '@/redux/modal/Actions';
import * as actionsGlobal from '@/redux/global/Actions';
import { RootState, AppDispatch } from '@/redux/Store';
import { IKeHoach, IResult, IDanhMucChiPhi, LoaiNhapLieuChiPhi, IChiTietDuToan, IPaginationResponse, LoaiBangChiPhi } from '@/models';
import { requestGET, requestPOST, requestPUT, API_URL, FILE_URL } from '@/utils/baseAPI';
import { FileUpload, TDHotTable, TDSelect } from '@/app/components';
...
export const KeHoachDetailModal = props => {
    ...
    try {
      ...
      const formData = {
        ...
        chiTietDuToans: chiTietData.map((item: any) => ({
          id: item.isCustom ? undefined : (isAdd ? undefined : item.id),
          keHoachId: id || null,
          danhMucChiPhiId: item.isCustom ? null : (item.danhMucChiPhiId || item.id),
          stt: item.stt,
          ten: item.ten,
          kyHieu: item.kyHieu,
          dinhMuc: item.dinhMuc,
          dinhMucNhapLieu: item.dinhMucNhapLieu,
          chiPhiTruocThue: Number(item.chiPhiTruocThue) || 0,
