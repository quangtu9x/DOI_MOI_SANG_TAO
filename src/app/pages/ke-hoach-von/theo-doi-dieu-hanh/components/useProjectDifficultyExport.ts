...
export const exportProjectDifficultiesToExcel = async (
  ...
  const rows = difficulties.map(difficulty => {
    ...
    visibleColumns.forEach(col => {
      switch (col.key) {
        case 'type':
          row[col.title] = getTypeLabel(difficulty.type);
          break;
        case 'level':
          row[col.title] = getLevelLabel(difficulty.level);
          break;
        case 'resolutionStatus':
          row[col.title] = getResolutionStatusLabel(difficulty.resolutionStatus);
          break;
        case 'occurredDate':
        case 'resolvedDate':
          row[col.title] = difficulty[col.key as keyof IProjectDifficulty]
            ? new Date(difficulty[col.key as keyof IProjectDifficulty] as string).toLocaleDateString('vi-VN')
            : '-';
          break;
        default:
          row[col.title] = difficulty[col.key as keyof IProjectDifficulty] || '-';
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'Danh sách khó khăn vướng mắc');

  // Generate filename
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
  const filename = `Danh_sach_kho_khan_vuong_mac_${timestamp}.xlsx`;

  // Download file
  XLSX.writeFile(wb, filename);
};

export const exportProjectDifficultiesToPDF = async (
  difficulties: IProjectDifficulty[],
  searchData?: any
): Promise<void> => {
  const columnConfig = getColumnConfig();
  ...
  let tableHTML = `
    <div style="text-align: center; margin-bottom: 20px;">
      <h2 style="font-size: 18px; font-weight: bold;">DANH SÁCH KHÓ KHĂN, VƯỚNG MẮC CỦA DỰ ÁN</h2>
    </div>
  ...
  difficulties.forEach((difficulty, index) => {
    ...
    visibleColumns.forEach(col => {
      ...
      switch (col.key) {
        case 'type':
          cellValue = getTypeLabel(difficulty.type);
          textAlign = 'center';
          break;
        case 'level':
          cellValue = getLevelLabel(difficulty.level);
          textAlign = 'center';
          break;
        case 'resolutionStatus':
          cellValue = getResolutionStatusLabel(difficulty.resolutionStatus);
          textAlign = 'center';
          break;
        case 'occurredDate':
        case 'resolvedDate':
          cellValue = difficulty[col.key as keyof IProjectDifficulty]
            ? new Date(difficulty[col.key as keyof IProjectDifficulty] as string).toLocaleDateString('vi-VN')
            : '-';
          textAlign = 'center';
          break;
        default:
          cellValue = String(difficulty[col.key as keyof IProjectDifficulty] || '-');
      }
  ...
  try {
    ...
    // Generate filename
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
    const filename = `Danh_sach_kho_khan_vuong_mac_${timestamp}.pdf`;
