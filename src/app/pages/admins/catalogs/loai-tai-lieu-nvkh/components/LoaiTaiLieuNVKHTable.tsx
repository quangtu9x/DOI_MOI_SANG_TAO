...
import { LoaiTaiLieuNVKHDetailModal } from './LoaiTaiLieuNVKHDetailModal';
import { useLoaiTaiLieuNVKHTable } from './useLoaiTaiLieuNVKHTable';

interface CategoryTableProps {
  searchData?: SearchData;
}

export const LoaiTaiLieuNVKHTable: React.FC<CategoryTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize, categoryGroupId } = useLoaiTaiLieuNVKHTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <LoaiTaiLieuNVKHDetailModal totalCount={totalCount} categoryGroupId={categoryGroupId} /> : <></>}
    </>
  );
};
