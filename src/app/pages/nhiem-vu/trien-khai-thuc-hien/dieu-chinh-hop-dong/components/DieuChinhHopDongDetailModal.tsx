...
import { IPaginationResponse, IDieuChinhHopDong, IResult } from '@/models';
...
export const DieuChinhHopDongDetailModal = props => {
  const dispatch: AppDispatch = useDispatch();
  const { totalCount } = props
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IDieuChinhHopDong | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IDieuChinhHopDong>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IDieuChinhHopDong>>(`DieuChinhHopDongs/${id}`);
        const _data = response?.data?.data ?? null;
        if (_data) {
          setDinhKem(handleImage(_data?.dinhKem ?? ''));
          _data.ngayKyPhuLuc = toViewDate(_data.ngayKyPhuLuc);
          _data.thoiGianKetThucMoi = toViewDate(_data.thoiGianKetThucMoi);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const formData: IDieuChinhHopDong = {
        ...values,
        ...(id && { id }),
        ngayKyPhuLuc: toSaveDate(values.ngayKyPhuLuc),
        thoiGianKetThucMoi: toSaveDate(values.thoiGianKetThucMoi),
        dinhKem: handleFiles(dinhKem).join('##'),
      };

      const response = id
        ? await requestPUT<IResult<string>>(`DieuChinhHopDongs/${id}`, formData)
        : await requestPOST<IResult<string>>(`DieuChinhHopDongs`, formData);
            ...
            <Form<IDieuChinhHopDong>
              ...
              <div className="row">
                ...
                <div className="col-xl-4 col-lg-4">
                  <Form.Item label="Số phụ lục" name="soPhuLucHopDong"
                    rules={[
                ...
                <div className="col-xl-4 col-lg-4">
                  <Form.Item label="Ngày ký" name='ngayKyPhuLuc' initialValue={dayjs()}>
                    <DatePicker placeholder='' format="DD/MM/YYYY" style={{ width: '100%' }} />
                  </Form.Item>
                </div>
                <div className="col-xl-4 col-lg-4">
                  <Form.Item label="Ngày kết thúc sau điều chỉnh" name='thoiGianKetThucMoi' initialValue={dayjs()}>
                    <DatePicker placeholder='' format="DD/MM/YYYY" style={{ width: '100%' }} />
                  </Form.Item>
                </div>
                <div className="col-xl-4 col-lg-4">
                  <Form.Item label="Tổng kinh phí sau điều chỉnh" name="tongKinhPhiMoi"
                    rules={[
                ...
                <div className="col-xl-4 col-lg-4">
                  <Form.Item label="Lý do điều chỉnh" name="lyDoDieuChinh">
                    <Input placeholder=' ' />
                  </Form.Item>
                </div>
                <div className="col-xl-4 col-lg-4">
