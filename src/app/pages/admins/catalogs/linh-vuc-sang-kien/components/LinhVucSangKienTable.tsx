...
import { LinhVucSangKienDetailModal } from './LinhVucSangKienDetailModal';
import { useLinhVucSangKienTable } from './useLinhVucSangKienTable';

interface CategoryTableProps {
  searchData?: SearchData;
}

export const LinhVucSangKienTable: React.FC<CategoryTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize, categoryGroupId } = useLinhVucSangKienTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <LinhVucSangKienDetailModal totalCount={totalCount} categoryGroupId={categoryGroupId} /> : <></>}
    </>
  );
};
