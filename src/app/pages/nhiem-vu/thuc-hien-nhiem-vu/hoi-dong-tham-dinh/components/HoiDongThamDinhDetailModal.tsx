...
import { IHoiDongThamDinh, IPaginationResponse, IResult, IChuyenGia } from '@/models';
...
export const HoiDongThamDinhDetailModal = props => {
  const dispatch: AppDispatch = useDispatch();
  const { totalCount } = props
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IHoiDongThamDinh | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IHoiDongThamDinh>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IHoiDongThamDinh>>(`HoiDongThamDinhs/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const formData: IHoiDongThamDinh = {
        ...values,
        ...(id && { id }),
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`HoiDongThamDinhs/${id}`, formData)
        : await requestPOST<IResult<string>>(`HoiDongThamDinhs`, formData);
