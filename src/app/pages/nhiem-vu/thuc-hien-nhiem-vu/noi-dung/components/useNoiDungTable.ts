...
import { IPaginationResponse, IHoSoThamDinh, LoaiHoSoThamDinh } from '@/models';
...
export const useHoSoThamDinhTable = ({ searchData, initialPageSize = 50 }: UseDataTableProps) => {
  ...
  const [data, setData] = useState<IHoSoThamDinh[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await requestPOST<IPaginationResponse<IHoSoThamDinh[]>>('HoSoThamDinhs/search', {
        ...
        pageSize,
        ...searchData,
        loaiHoSoThamDinh: LoaiHoSoThamDinh.NoiDung,
      });
