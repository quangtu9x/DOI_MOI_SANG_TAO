...
import { DoiTuongNopSangKienDetailModal } from './DoiTuongNopSangKienDetailModal';
import { useDoiTuongNopSangKienTable } from './useDoiTuongNopSangKienTable';

interface CategoryTableProps {
  searchData?: SearchData;
}

export const DoiTuongNopSangKienTable: React.FC<CategoryTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize, categoryGroupId } = useDoiTuongNopSangKienTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <DoiTuongNopSangKienDetailModal totalCount={totalCount} categoryGroupId={categoryGroupId} /> : <></>}
    </>
  );
};
