...
export const ContractorDetailModal = () => {
  const dispatch: AppDispatch = useDispatch();
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IContractor | null;
  ...
  const onFinish = async (): Promise<void> => {
    ...
    try {
      ...
      const formData: ICreateContractorRequest | IUpdateContractorRequest = {
        ...(id && { id }),
        name: values.name,
        code: values.code,
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`Contractors/${id}`, formData)
        : await requestPOST<IResult<string>>(`Contractors`, formData);
  ...
  return (
    ...
    >
      <Modal.Header className="bg-primary px-4 py-3">
        <Modal.Title className="text-white">{dataModal?.readOnly ? 'Chi tiết' : id ? 'Chỉnh sửa' : 'Tạo mới'} nhà thầu</Modal.Title>
