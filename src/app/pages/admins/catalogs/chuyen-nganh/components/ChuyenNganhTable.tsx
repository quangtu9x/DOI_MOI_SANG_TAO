...
import { ChuyenNganhDetailModal } from './ChuyenNganhDetailModal';
import { useChuyenNganhTable } from './useChuyenNganhTable';

interface CategoryTableProps {
  searchData?: SearchData;
}

export const ChuyenNganhTable: React.FC<CategoryTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize, categoryGroupId } = useChuyenNganhTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <ChuyenNganhDetailModal totalCount={totalCount} categoryGroupId={categoryGroupId} /> : <></>}
    </>
  );
};
