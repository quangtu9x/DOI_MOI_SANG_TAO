...
import { PhieuDeNghiThanhToanTable } from './components/PhieuDeNghiThanhToanTable';
...
const buildPrintHtml = (rows: unknown[][]): string => {
  ...
  return `
    ...
    <html>
      <head>
        <meta charset="utf-8" />
        <title>In danh sách phiếu đề nghị thanh toán</title>
        <style>
          @page {
            size: A4 landscape;
            margin: 12mm;
      ...
      <body>
        <h2>Danh sách phiếu đề nghị thanh toán</h2>
        <table>
          <thead>
...
const printHtml = (html: string): void => {
  ...
  printFrame.title = 'In danh sách phiếu đề nghị thanh toán';
...
export const TraCuuPhieuDeNghiThanhToanPage = () => {
  ...
  const handleExport = async (): Promise<void> => {
    try {
      setIsExporting(true);
      const response = await requestDownloadFile('PhieuDeNghiThanhToans/export', buildExportPayload(searchData));
      ...
      <Content>
        <div className="card card-xl-stretch mb-xl-9">
          <div className="px-3 py-3 border-bottom border-secondary border-bottom-solid d-flex align-items-center justify-content-between">
            <h3 className="card-title fw-bold text-header-td fs-4 mb-0">{'Tra cứu phiếu đề nghị thanh toán'}</h3>
          ...
          <PhieuDeNghiThanhToanTable searchData={searchData} readOnly={true} />
        </div>
      </Content>
    </>
  );
