...
import { IResult, IChuyenGiaGiaiThuong } from '@/models';
import { TDTable } from '@/app/components';
import { useGiaiThuongTable } from './useGiaiThuongTable';
import { GiaiThuongDetailModal } from './GiaiThuongDetailModal';
...
export const GiaiThuongTable: React.FC<DataTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useGiaiThuongTable({ searchData });

  const handleAction = useCallback(
    async (type: string, record: IChuyenGiaGiaiThuong): Promise<void> => {
      try {
        switch (type) {
          case 'detail':
            ...
            break;
          case 'delete':
            const response = await requestDELETE<IResult<boolean>>(`chuyengias/${record.id}/giaithuong`);
  ...
  const columns: TableProps<IChuyenGiaGiaiThuong>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
  ...
  return (
    <>
      ...
      {modalVisible ? <GiaiThuongDetailModal /> : <></>}
    </>
  );
};
