...
import { ChuongTrinhNVKHDetailModal } from './ChuongTrinhNVKHDetailModal';
import { useChuongTrinhNVKHTable } from './useChuongTrinhNVKHTable';

interface CategoryTableProps {
  searchData?: SearchData;
}

export const ChuongTrinhNVKHTable: React.FC<CategoryTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize, categoryGroupId } = useChuongTrinhNVKHTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <ChuongTrinhNVKHDetailModal totalCount={totalCount} categoryGroupId={categoryGroupId} /> : <></>}
    </>
  );
};
