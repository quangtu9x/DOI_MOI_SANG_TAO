...
import { IResult, IDotDangKy } from '@/models';
import { TDTable } from '@/app/components';
import { useDotDangKyTable } from './useDotDangKyTable';
import { DotDangKyDetailModal } from './DotDangKyDetailModal';
import { toViewDate, toViewDateString } from '@/utils/utils';

interface DotDangKyTableProps {
  searchData?: SearchData;
}

export const DotDangKyTable: React.FC<DotDangKyTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useDotDangKyTable({ searchData });

  const handleAction = useCallback(
    async (type: string, record: IDotDangKy): Promise<void> => {
      try {
        switch (type) {
          case 'detail':
            ...
            break;
          case 'delete':
            const response = await requestDELETE<IResult<boolean>>(`DotDangKys/${record.id}`);
  ...
  const columns: TableProps<IDotDangKy>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
  ...
  return (
    <>
      ...
      {modalVisible ? <DotDangKyDetailModal totalCount={totalCount} /> : <></>}
    </>
  );
};
