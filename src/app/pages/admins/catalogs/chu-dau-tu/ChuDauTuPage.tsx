...
import { ChuDauTuTable } from './components/ChuDauTuTable';
import { Select } from 'antd';
import { CATEGORY_GROUP_CODE } from '@/data';

export const ChuDauTuPage = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await requestPOST<IPaginationResponse<ICategoryGroup[]>>(`categorygroups/search`, {
          ...
          code: CATEGORY_GROUP_CODE.CHU_DAU_TU,
          pageNumber: 1,
          pageSize: 1000,
        });
        if (response.data) {
      ...
      <Content>
        <div className="card card-xl-stretch mb-xl-9">
          <div className="px-3 py-3 border-bottom border-secondary border-bottom-solid d-flex align-items-center justify-content-between">
            <h3 className="card-title fw-bold text-header-td fs-4 mb-0">{'Danh mục chủ đầu tư'}</h3>
          ...
          <ChuDauTuTable searchData={searchData} />
        </div>
      </Content>
    </>
  );
