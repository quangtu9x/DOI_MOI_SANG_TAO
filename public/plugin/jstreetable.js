...
(function (factory) {
  ...
  var renderATitle,
    ...
    NODE_DATA_ATTR = "data-jstreetable",
    COL_DATA_ATTR = "data-jstreetable-column",
    SPECIAL_TITLE = "_DATA_",
    LEVELINDENT = 24,
    styled = false,
    TABLECELLID_PREFIX = "jstable_",
    TABLECELLID_POSTFIX = "_col",
    MINCOLWIDTH = 10,
  ...
  copyData = function (fromtree, from, totree, to, recurse) {
    ...
    if (from && from.children_d && recurse) {
      for (i = 0, j = from.children_d.length; i < j; i++) {
        copyData(
          fromtree,
          fromtree.get_node(from.children_d[i]),
          totree,
          totree.get_node(to.children_d[i]),
          recurse
        );
      }
    }
  ...
  renderAWidth = function (node, tree) {
    var depth,
      width,
      fullWidth =
        parseInt(tree.settings.table.columns[0].width, 10) +
        parseInt(tree._tableSettings.treeWidthDiff, 10);
  ...
  renderATitle = function (node, t, tree) {
    var a =
        node.get(0).tagName.toLowerCase() === "a" ? node : node.children("a"),
      title,
      col = tree.settings.table.columns[0];
    // get the title
    title = "";
    if (col.title) {
  ...
  $.jstree.plugins.table = function (options, parent) {
    ...
    this._initialize = function () {
      if (!this._initialized) {
        ...
        this.tableWrapper = $("<div></div>")
          .addClass("jstree-table-wrapper")
          .insertAfter(container);
        this.headerWrapper = $("<div></div>")
          .addClass("jstree-table-headerwrapper")
          .appendTo(this.tableWrapper);
        this.midWrapper = $("<div></div>")
        ...
        for (i = 0; i < cols.length; i++) {
          ...
          $("<div></div>")
            .addClass(
              "jstree-default jstree-table-column jstree-table-column-" +
                i +
                " jstree-table-column-root-" +
                this.rootid
            )
            .appendTo(this.midWrapper);
        }
    ...
    this.bind = function () {
      ...
      this.element
        .on(
          "move_node.jstree create_node.jstree clean_node.jstree change_node.jstree",
        ...
        .on(
          ...
          $.proxy(function (e, data) {
            ...
            function resize() {
              ...
              var anchorHeight =
                me.element.find(".jstree-leaf").outerHeight() || 24;
              ...
              $(
                '<style type="text/css" id="jsTreeTableExtraCss">\
                  ...
                  anchorHeight +
                  "px;}\
						div.jstree-table-midwrapper a.jstree-clicked:before, .jstree-table-midwrapper a.jstree-hovered:before {width: " +
                  tableWidth +
                  "px;}\
					</style>"
              ).appendTo("head");
        ...
        .on(
          "select_node.jstree",
          $.proxy(function (node, selected, event) {
            var id = selected.node.id;
            findDataCell(this.tableWrapper, id).addClass("jstree-clicked");
            this.get_node(selected.node.id, true)
              .children("div.jstree-table-cell")
        ...
        .on(
          ...
          $.proxy(function (node, selected, event) {
            ...
            for (i = 0; i < ids.length; i++) {
              findDataCell(this.tableWrapper, ids[i]).removeClass(
                "jstree-clicked"
              );
            }
          }, this)
          ...
          $.proxy(function (e, data) {
            ...
            if (this._data.search.som) {
              if (data.nodes.length) {
                ...
                data.nodes
                  .add(data.nodes.parentsUntil(".jstree"))
                  .filter(".jstree-node")
      ...
      if (this._tableSettings.stateful) {
        this.element.on(
          "resize_column.jstree-table",
          $.proxy(function (e, col, width) {
            localStorage["jstree-root-" + this.rootid + "-column-" + col] =
              width;
          }, this)
        );
      }
    ...
    this.teardown = function () {
      var gw = this.tableWrapper,
        container = this.element,
        tableparent = gw.parent();
      container.detach();
      gw.remove();
    ...
    this._prepare_headers = function () {
      ...
      for (i = 0; i < cols.length; i++) {
        ...
        if (
          gs.stateful &&
          localStorage["jstree-root-" + rootid + "-column-" + i]
        )
          width = localStorage["jstree-root-" + rootid + "-column-" + i];
        else width = cols[i].width || defaultWidth;
        ...
        last = $("<div></div>")
          ...
          .addClass(
            "jstree-table-div-" +
              ...
              " " +
              (tr ? "ui-widget-header " : "") +
              " jstree-table-header jstree-table-header-cell jstree-table-header-" +
              classAdd +
              " " +
              cl +
              " " +
        ...
        last.addClass(
          (tr ? "ui-widget-header " : "") +
            "jstree-table-header jstree-table-header-" +
            classAdd
        );
        last.appendTo(col);
        ...
        puller = $(
          "<div class='jstree-table-separator jstree-table-separator-" +
            classAdd +
            (tr ? " ui-widget-header" : "") +
            (resizable ? " jstree-table-resizable-separator" : "") +
            "'>&nbsp;</div>"
        ).appendTo(last);
      }
      ...
      last.addClass(
        (tr ? "ui-widget-header " : "") +
          "jstree-table-header jstree-table-header-last jstree-table-header-" +
          classAdd
      );
      ...
      if (cols[cols.length - 1].width === undefined) {
        ...
        col.css({ width: "auto" });
        last
          .addClass("jstree-table-width-auto")
          .next(".jstree-table-separator")
          .remove();
      }
      if (hasHeaders) {
      ...
      if (!this.bound && resizable) {
        ...
        $(document)
          .mouseup(function () {
            ...
            if (isClickedSep) {
              ...
              currentTree = toResize
                .closest(".jstree-table-wrapper")
                .find(".jstree");
              ...
              if (isNaN(colNum) || colNum < 0) {
                ref._tableSettings.treeWidthDiff =
                  currentTree.find("ins:eq(0)").width() +
                  currentTree.find("a:eq(0)").width() -
                  ref._tableSettings.columns[0].width;
              }
              width = ref._tableSettings.columns[colNum].width = parseFloat(
                toResize.css("width")
              );
              isClickedSep = false;
              toResize = null;

              currentTree.trigger("resize_column.jstree-table", [
                colNum,
                width,
              ]);
            }
          ...
          .mousemove(function (e) {
            if (isClickedSep) {
              ...
              if (diff !== 0) {
                ...
                if (
                  (diff > 0 || oldPrevHeaderInner > 0) &&
                  newPrevColWidth > MINCOLWIDTH
                ) {
        ...
        this.tableWrapper
          .on("selectstart", ".jstree-table-resizable-separator", function () {
            return false;
          })
      ...
      this.tableWrapper.on("click", ".jstree-table-header-cell", function (e) {
        ...
        $(this)
          .closest(".jstree-table-wrapper")
          .find(".jstree-table-sort-icon")
          .remove();
        $("<span></span>")
          .addClass("jstree-table-sort-icon")
          .appendTo($(this))
          .addClass(arrowClass);
      ...
      // header context menu
      this.headerWrapper.on(
        "contextmenu",
        ".jstree-table-header-cell",
        function (e) {
          if (!gs.headerContextMenu) {
            return;
          }
          e.preventDefault();
          ...
          _this.midWrapper.find(".jstree-table-header-cell").each(function () {
            ...
            label = cell
              .clone()
              .children(".jstree-table-sort-icon")
              .remove()
              .end()
              .text()
              .trim();
            ...
            options[value] = {
              ...
              column: value,
              label: label,
              _disabled: value === "text",
              action: function (data) {
                var col = _this.midWrapper
                  .find(
                    ".jstree-table-header-cell[" +
                      COL_DATA_ATTR +
                      "='" +
        ...
        if (old !== undefined) {
          ...
          for (i = 0; i < oldNodes.length; i++) {
            findDataCell(table, oldNodes[i])
              ...
              .attr(
                "id",
                TABLECELLID_PREFIX + obj.id + TABLECELLID_POSTFIX + (i + 1)
              )
    ...
    this._hide_table = function (node) {
      var children = node && node.children_d ? node.children_d : [],
        i;
    ...
    this.getHoldingCells = function (obj, col, hc) {
      ...
      for (i = 0; i < children.length; i++) {
        ...
        if (hc[child] && obj.state.opened) {
          ret = ret
            .add(hc[child])
            .add(this.getHoldingCells(this.get_node(children[i]), col, hc));
          //delete hc[child];
        }
      }
      return ret;
        ...
        h2 = $("<" + "input />", {
          ...
          blur: $.proxy(function () {
            ...
            if (v === "" || v === t) {
              ...
              this.element.trigger("update_cell.jstree-table", {
                node: obj,
                col: col.value,
                value: v,
                old: t,
    ...
    this.autosize_column = function (col) {
      ...
      var oldPrevColWidth = parseFloat(col.css("width")),
        newWidth = 0,
        diff,
      ...
      $(this)
        .closest(".jstree-table-wrapper")
        .find(".jstree")
        .trigger("resize_column.jstree-table", [colNum, newPrevColWidth]);
    };
    ...
    this._prepare_table = function (obj) {
      ...
      if (a.length === 1) {
        ...
        tableCellName =
          TABLECELLID_PREFIX + escapeId(lid) + TABLECELLID_POSTFIX;
        ...
        for (i = 0; i < cols.length; i++) {
          ...
          valClass =
            col.valueClass &&
            objData.data !== null &&
            objData.data !== undefined
              ? objData.data[col.valueClass] || ""
              : "";
          ...
          wideValClass =
            ...
            objData.data !== null &&
            objData.data !== undefined
              ? objData.data[col.wideValueClass] || ""
              : "";
          if (
            wideValClass &&
            col.wideValueClassPrefix &&
            col.wideValueClassPrefix !== ""
          ) {
          ...
          // get the title
          title =
            col.title && objData.data !== null && objData.data !== undefined
              ? objData.data[col.title] || ""
              : "";
          // strip out HTML
          ...
          tableCellPrevId =
            pos <= 0
              ? objData.parent
              : findLastClosedNode(this, peers[pos - 1]);
          ...
          last = last
            .css(conf)
            .addClass(
              "jstree-table-cell jstree-table-cell-regular jstree-table-cell-root-" +
                rootid +
                " jstree-table-cell-" +
                classAdd +
                " " +
          ...
          last.on(
            "contextmenu",
            cellRightClickHandler(tree, t, val, col, this)
          );
        ...
        last.addClass(
          "jstree-table-cell-last" + (tr ? " ui-state-default" : "")
        );
        // if there is no width given for the last column, do it via automatic
        if (cols[cols.length - 1].width === undefined) {
          last
