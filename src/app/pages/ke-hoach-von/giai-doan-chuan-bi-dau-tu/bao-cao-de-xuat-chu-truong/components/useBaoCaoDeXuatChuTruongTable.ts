...
interface UseBaoCaoDeXuatChuTruongTableProps {
  searchData?: SearchData;
  projectProcessExecutionId?: string;
  initialPageSize?: number;
}

export const useBaoCaoDeXuatChuTruongTable = ({
  searchData,
  projectProcessExecutionId,
  initialPageSize = 50,
}: UseBaoCaoDeXuatChuTruongTableProps) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const searchRequest: ISearchProjectProcessStepExecutionRequest = {
        ...
        stepType: StepType.InvestmentPolicyProposal, // Filter theo báo cáo đề xuất chủ trương đầu tư
        ...
        if (responseData && responseData.length > 0) {
          ...
          if (uniqueStepIds.length > 0) {
            try {
              ...
              console.error('[useBaoCaoDeXuatChuTruongTable] Error fetching steps:', error);
            }
          }
          ...
          if (uniqueUserIds.length > 0) {
            try {
              ...
              console.error('[useBaoCaoDeXuatChuTruongTable] Error fetching users:', error);
            }
          }
          ...
          const enrichedData = responseData.map((item) => {
            ...
            if (item.projectProcessStepId) {
              ...
              if (step) {
                ...
                console.warn(`[useBaoCaoDeXuatChuTruongTable] Step not found for ID: ${item.projectProcessStepId}`);
              }
            }
            ...
            if (item.assignedUserId) {
              ...
              if (user) {
                ...
                console.warn(`[useBaoCaoDeXuatChuTruongTable] User not found for ID: ${item.assignedUserId}`);
              }
            }
