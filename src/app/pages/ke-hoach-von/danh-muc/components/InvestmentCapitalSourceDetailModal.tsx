...
import {
  getInvestmentCapitalSourceById,
} from '@/services/investmentCapitalSource.service';

export const InvestmentCapitalSourceDetailModal = () => {
  const dispatch: AppDispatch = useDispatch();
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IInvestmentCapitalSource | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IInvestmentCapitalSource>();
  ...
  useEffect(() => {
    const loadData = async (): Promise<void> => {
      ...
      try {
        setIsLoading(true);
        const response = await getInvestmentCapitalSourceById(id);
        if (response) {
  ...
  const onFinish = async (): Promise<void> => {
    ...
    try {
      ...
      const formData: ICreateInvestmentCapitalSourceRequest | IUpdateInvestmentCapitalSourceRequest = {
        ...(id && { id }),
        name: values.name,
        code: values.code,
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`InvestmentCapitalSources/${id}`, formData)
        : await requestPOST<IResult<string>>(`InvestmentCapitalSources`, formData);
  ...
  return (
    ...
    >
      <Modal.Header className="bg-primary px-4 py-3">
        <Modal.Title className="text-white">{dataModal?.readOnly ? 'Chi tiết' : id ? 'Chỉnh sửa' : 'Tạo mới'} nguồn vốn đầu tư</Modal.Title>
        <button type="button" className="btn-close btn-close-white" aria-label="Close" onClick={handleCancel}></button>
      </Modal.Header>
      <Modal.Body>
        <Spin spinning={isLoading}>
          <Form<IInvestmentCapitalSource>
            form={form}
            layout="vertical"
            autoComplete="off"
            ...
            <div className="row">
              <div className="col-xl-6 col-lg-6">
                <Form.Item
                  ...
                  name="code"
                >
                  <Input placeholder="Mã nguồn vốn đầu tư" />
                </Form.Item>
              </div>
              <div className="col-xl-6 col-lg-6">
                <Form.Item
                  label="Tên"
                  name="name"
                  rules={[{ required: true, message: 'Vui lòng nhập tên nguồn vốn đầu tư' }]}
                >
                  <Input placeholder="Tên nguồn vốn đầu tư" />
                </Form.Item>
              </div>
              <div className="col-xl-6 col-lg-6">
