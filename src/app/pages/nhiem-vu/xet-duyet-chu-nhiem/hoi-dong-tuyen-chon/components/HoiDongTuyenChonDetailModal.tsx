...
import { IHoiDongTuyenChon, IPaginationResponse, IResult, IChuyenGia, IUserDetails } from '@/models';
...
export const HoiDongTuyenChonDetailModal = props => {
  const dispatch: AppDispatch = useDispatch();
  const { totalCount } = props
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IHoiDongTuyenChon | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IHoiDongTuyenChon>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IHoiDongTuyenChon>>(`HoiDongTuyenChons/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const formData: IHoiDongTuyenChon = {
        ...values,
        ...(id && { id }),
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`HoiDongTuyenChons/${id}`, formData)
        : await requestPOST<IResult<string>>(`HoiDongTuyenChons`, formData);
                                    ...
                                    fetchUsers={async (keyword: string) => {
                                      ...
                                      return (
                                        res.data?.data?.map(item => ({
                                          ...item,
                                          label: `${item.fullName} ${item?.positionName ? `(${item?.positionName})` : ''}`,
                                          value: item?.id,
                                        })) ?? []
                                      );
                                    }}
