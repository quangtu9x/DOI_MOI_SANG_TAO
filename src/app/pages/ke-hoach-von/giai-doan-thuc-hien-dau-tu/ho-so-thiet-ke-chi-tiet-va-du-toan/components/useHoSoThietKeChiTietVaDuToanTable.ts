...
interface UseHoSoThietKeChiTietVaDuToanTableProps {
  searchData?: SearchData;
  projectProcessExecutionId?: string;
  initialPageSize?: number;
}

export const useHoSoThietKeChiTietVaDuToanTable = ({
  searchData,
  projectProcessExecutionId,
  initialPageSize = 50,
}: UseHoSoThietKeChiTietVaDuToanTableProps) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const searchRequest: ISearchProjectProcessStepExecutionRequest = {
        ...
        stepTypes: [StepType.DetailedDesignPreparation, StepType.DetailedDesignAppraisal], // Filter theo hồ sơ thiết kế chi tiết và dự toán (Lập hồ sơ và Thẩm định)
        phase: ProjectPhase.Implementation,         // Filter theo giai đoạn thực hiện đầu tư
        ...searchData,
      };
        ...
        if (responseData && responseData.length > 0) {
          ...
          if (uniqueStepIds.length > 0) {
            try {
              const stepRes = await searchProjectProcessSteps({
                pageNumber: 1,
                pageSize: 10000,
                stepTypes: [StepType.DetailedDesignPreparation, StepType.DetailedDesignAppraisal],
              });
              ...
              console.error('[useHoSoThietKeChiTietVaDuToanTable] Error fetching steps:', error);
            }
          }
          ...
          if (uniqueUserIds.length > 0) {
            try {
              ...
              console.error('[useHoSoThietKeChiTietVaDuToanTable] Error fetching users:', error);
            }
          }
          ...
          const enrichedData = responseData.map((item) => {
            ...
            if (item.projectProcessStepId) {
              ...
              if (step) {
                ...
                console.warn(`[useHoSoThietKeChiTietVaDuToanTable] Step not found for ID: ${item.projectProcessStepId}`);
              }
            }
            ...
            if (item.assignedUserId) {
              ...
              if (user) {
                ...
                console.warn(`[useHoSoThietKeChiTietVaDuToanTable] User not found for ID: ${item.assignedUserId}`);
              }
            }
