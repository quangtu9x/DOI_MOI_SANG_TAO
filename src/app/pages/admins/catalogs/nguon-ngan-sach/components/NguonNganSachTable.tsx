...
import { NguonNganSachDetailModal } from './NguonNganSachDetailModal';
import { useNguonNganSachTable } from './useNguonNganSachTable';

interface CategoryTableProps {
  searchData?: SearchData;
}

export const NguonNganSachTable: React.FC<CategoryTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize, categoryGroupId } = useNguonNganSachTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <NguonNganSachDetailModal totalCount={totalCount} categoryGroupId={categoryGroupId} /> : <></>}
    </>
  );
};
