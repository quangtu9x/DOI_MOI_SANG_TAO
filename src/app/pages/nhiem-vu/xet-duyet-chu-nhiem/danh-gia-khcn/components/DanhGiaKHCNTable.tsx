...
import { useDanhGiaKHCNTable } from './useDanhGiaKHCNTable';
import { DanhGiaKHCNDetailModal } from './DanhGiaKHCNDetailModal';
import { toViewDateString } from '@/utils/utils';

interface DanhGiaKHCNTableProps {
  searchData?: SearchData;
}

export const DanhGiaKHCNTable: React.FC<DanhGiaKHCNTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const modalVisibleCapMot = useSelector((state: RootState) => state.modal.modalVisibleCapMot);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useDanhGiaKHCNTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <DanhGiaKHCNDetailModal totalCount={totalCount} /> : <></>}
    </>
  );
};
