...
import { IResult, IThuyetMinhSXTN } from '@/models';
import { TDTable } from '@/app/components';
import { useThuyetMinhSXTNTable } from './useThuyetMinhSXTNTable';
import { ThuyetMinhSXTNDetailModal } from './ThuyetMinhSXTNDetailModal';
import { toViewDateString } from '@/utils/utils';
import { PHUONG_THUC_KHOAN } from '@/data';

interface ThuyetMinhSXTNTableProps {
  searchData?: SearchData;
}

export const ThuyetMinhSXTNTable: React.FC<ThuyetMinhSXTNTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useThuyetMinhSXTNTable({ searchData });

  const handleAction = useCallback(
    async (type: string, record: IThuyetMinhSXTN): Promise<void> => {
      try {
        switch (type) {
          case 'detail':
            ...
            break;
          case 'delete':
            const response = await requestDELETE<IResult<boolean>>(`ThuyetMinhSXTNs/${record.id}`);
  ...
  const columns: TableProps<IThuyetMinhSXTN>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
  ...
  return (
    <>
      ...
      {modalVisible ? <ThuyetMinhSXTNDetailModal totalCount={totalCount} /> : <></>}
    </>
  );
};
