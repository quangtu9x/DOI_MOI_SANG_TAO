...
import { Avatar, DatePicker, Form, Input, InputNumber, Radio, Space, Spin } from 'antd';
import { Modal, Button } from 'react-bootstrap';

import * as actionsModal from '@/redux/modal/Actions';
import * as actionsGlobal from '@/redux/global/Actions';
import { RootState, AppDispatch } from '@/redux/Store';
import { IThuyetMinhXHNV, IPaginationResponse, IResult, PhuongThucKhoan, IChuyenGia, LoaiNhiemVu, IUserDetails } from '@/models';
...
export const ThuyetMinhXHNVDetailModal = props => {
  const dispatch: AppDispatch = useDispatch();
  const { totalCount } = props
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IThuyetMinhXHNV | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IThuyetMinhXHNV>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IThuyetMinhXHNV>>(`ThuyetMinhXHNVs/${id}`);
        ...
        if (_data) {
          ...
          _data.tinhTrangNhiemVu = _data.tinhTrangNhiemVuId ? {
            value: _data.tinhTrangNhiemVuId,
            label: _data.tinhTrangNhiemVuTen,
          } : null;
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const formData: IThuyetMinhXHNV = {
        ...values,
        ...(id && { id }),
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`ThuyetMinhXHNVs/${id}`, formData)
        : await requestPOST<IResult<string>>(`ThuyetMinhXHNVs`, formData);
            ...
            <Form<IThuyetMinhXHNV>
              ...
              <div className="row">
                ...
                <div className="col-xl-6 col-lg-6">
                  <Form.Item label="Tình trạng nhiệm vụ" name="tinhTrangNhiemVu" rules={[{ required: true, message: 'Không được để trống!' }]}>
                    <TDSelect
                      notFoundContent="Không tìm thấy dữ liệu"
                      reload
                      showSearch
                      ...
                      fetchOptions={async keyword => {
                        const res = await requestPOST<IPaginationResponse<any[]>>(`categories/search`, {
                          ...
                          pageSize: 1000,
                          keyword: keyword,
                          categoryGroupCode: CATEGORY_GROUP_CODE.TINH_TRANG_NHIEM_VU,
                        });
                        return (
                          res.data?.data?.map(item => ({
                            ...item,
                      ...
                      onChange={(value, current: any) => {
                        if (value) {
                          form.setFieldValue('tinhTrangNhiemVuId', current?.id);
                        } else {
                          form.setFieldValue('tinhTrangNhiemVuId', null);
                        }
                      }}
                    />
                  </Form.Item>
                ...
                <div className="col-xl-6 col-lg-6">
                  <Form.Item label="Hoạt động phục vụ nghiên cứu" name="hoatDongPhucVuNghienCuu">
                    <Input.TextArea rows={2} placeholder="" />
                  </Form.Item>
                </div>
                <div className="col-xl-6 col-lg-6">
                  <Form.Item label="Lợi ích nhiệm vụ" name="loiIchNhiemVu">
                    <Input.TextArea rows={2} placeholder="" />
                  </Form.Item>
                </div>
                <div className="col-xl-4 col-lg-4">
