...
import { IProjectProcess, ISearchProjectProcessRequest } from '@/models/ke-hoach-von';
import { SearchData } from '@/types';
import { AppDispatch, RootState } from '@/redux/Store';
import { searchProjectProcesses } from '@/services/projectProcess.service';

interface UseProjectProcessTableProps {
  searchData?: SearchData;
  initialPageSize?: number;
}

export const useProjectProcessTable = ({
  searchData,
  initialPageSize = 50,
}: UseProjectProcessTableProps) => {
  const dispatch: AppDispatch = useDispatch();
  const random = useSelector((state: RootState) => state.global.random);

  const [data, setData] = useState<IProjectProcess[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const searchRequest: ISearchProjectProcessRequest = {
      ...
      const response = await searchProjectProcesses(searchRequest);
