...
import { IPosition, IResult } from '@/models';
...
export const PositionDetailModal = props => {
  const dispatch: AppDispatch = useDispatch();
  const { totalCount } = props
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IPosition | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IPosition>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IPosition>>(`positions/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`Positions/${id}`, formData)
        : await requestPOST<IResult<string>>(`Positions`, formData);
