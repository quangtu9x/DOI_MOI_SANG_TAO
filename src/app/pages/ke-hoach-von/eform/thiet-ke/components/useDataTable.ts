...
import { IPaginationResponse, IEform } from '@/models';
...
export const useDataTable = ({ searchData, initialPageSize = 50 }: UseDataTableProps) => {
  ...
  const [data, setData] = useState<IEform[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const response = await requestPOST<IPaginationResponse<IEform[]>>('eforms/search', {
