...
interface UseBaoCaoKinhTeKyThuatTableProps {
  searchData?: SearchData;
  projectProcessExecutionId?: string;
  initialPageSize?: number;
}

export const useBaoCaoKinhTeKyThuatTable = ({
  searchData,
  projectProcessExecutionId,
  initialPageSize = 50,
}: UseBaoCaoKinhTeKyThuatTableProps) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const searchRequest: ISearchProjectProcessStepExecutionRequest = {
        ...
        stepType: StepType.TechnicalEconomicReport, // Filter theo báo cáo kinh tế kỹ thuật
        ...
        if (responseData && responseData.length > 0) {
          ...
          if (uniqueStepIds.length > 0) {
            try {
              ...
              console.error('[useBaoCaoKinhTeKyThuatTable] Error fetching steps:', error);
            }
          }
          ...
          if (uniqueUserIds.length > 0) {
            try {
              ...
              console.error('[useBaoCaoKinhTeKyThuatTable] Error fetching users:', error);
            }
          }
          ...
          const enrichedData = responseData.map((item) => {
            ...
            if (item.projectProcessStepId) {
              ...
              if (step) {
                ...
                console.warn(`[useBaoCaoKinhTeKyThuatTable] Step not found for ID: ${item.projectProcessStepId}`);
              }
            }
            ...
            if (item.assignedUserId) {
              ...
              if (user) {
                ...
                console.warn(`[useBaoCaoKinhTeKyThuatTable] User not found for ID: ${item.assignedUserId}`);
              }
            }
