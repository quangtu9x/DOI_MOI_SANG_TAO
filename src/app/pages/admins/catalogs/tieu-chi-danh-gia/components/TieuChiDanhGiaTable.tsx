...
import { TieuChiDanhGiaDetailModal } from './TieuChiDanhGiaDetailModal';
import { useTieuChiDanhGiaTable } from './useTieuChiDanhGiaTable';

interface CategoryTableProps {
  searchData?: SearchData;
}

export const TieuChiDanhGiaTable: React.FC<CategoryTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize, categoryGroupId } = useTieuChiDanhGiaTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <TieuChiDanhGiaDetailModal totalCount={totalCount} categoryGroupId={categoryGroupId} /> : <></>}
    </>
  );
};
