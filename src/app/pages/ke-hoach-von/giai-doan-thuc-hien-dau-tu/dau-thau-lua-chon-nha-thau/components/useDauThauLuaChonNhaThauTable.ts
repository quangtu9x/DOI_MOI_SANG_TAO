...
interface UseDauThauLuaChonNhaThauTableProps {
  searchData?: SearchData;
  projectProcessExecutionId?: string;
  initialPageSize?: number;
}

export const useDauThauLuaChonNhaThauTable = ({
  searchData,
  projectProcessExecutionId,
  initialPageSize = 50,
}: UseDauThauLuaChonNhaThauTableProps) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const searchRequest: ISearchProjectProcessStepExecutionRequest = {
        ...
        stepType: StepType.BiddingContractorSelection, // Filter theo đấu thầu, lựa chọn nhà thầu
        ...
        if (responseData && responseData.length > 0) {
          ...
          if (uniqueStepIds.length > 0) {
            try {
              ...
              console.error('[useDauThauLuaChonNhaThauTable] Error fetching steps:', error);
            }
          }
          ...
          if (uniqueUserIds.length > 0) {
            try {
              ...
              console.error('[useDauThauLuaChonNhaThauTable] Error fetching users:', error);
            }
          }
          ...
          const enrichedData = responseData.map((item) => {
            ...
            if (item.projectProcessStepId) {
              ...
              if (step) {
                ...
                console.warn(`[useDauThauLuaChonNhaThauTable] Step not found for ID: ${item.projectProcessStepId}`);
              }
            }
            ...
            if (item.assignedUserId) {
              ...
              if (user) {
                ...
                console.warn(`[useDauThauLuaChonNhaThauTable] User not found for ID: ${item.assignedUserId}`);
              }
            }
