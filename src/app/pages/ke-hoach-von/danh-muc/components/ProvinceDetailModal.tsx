...
export const ProvinceDetailModal = () => {
  const dispatch: AppDispatch = useDispatch();
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IProvinceCity | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IProvinceCity>();
  ...
  const onFinish = async (): Promise<void> => {
    ...
    try {
      ...
      const formData: ICreateProvinceCityRequest | IUpdateProvinceCityRequest = {
        ...(id && { id }),
        name: values.name,
        code: values.code,
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`ProvinceCities/${id}`, formData)
        : await requestPOST<IResult<string>>(`ProvinceCities`, formData);
  ...
  return (
    ...
    >
      <Modal.Header className="bg-primary px-4 py-3">
        <Modal.Title className="text-white">{dataModal?.readOnly ? 'Chi tiết' : id ? 'Chỉnh sửa' : 'Tạo mới'} tỉnh thành phố</Modal.Title>
            ...
            <div className="row">
              <div className="col-xl-6 col-lg-6">
                <Form.Item
                  ...
                  name="code"
                >
                  <Input placeholder="Mã tỉnh thành phố" />
                </Form.Item>
              </div>
              <div className="col-xl-6 col-lg-6">
                <Form.Item
              ...
              <div className="col-xl-6 col-lg-6">
                <Form.Item label="Cấp hành chính" name="level">
                  <Input type="number" placeholder="Cấp hành chính (1: Tỉnh, 2: Thành phố)" />
                </Form.Item>
              </div>
              <div className="col-xl-6 col-lg-6">
