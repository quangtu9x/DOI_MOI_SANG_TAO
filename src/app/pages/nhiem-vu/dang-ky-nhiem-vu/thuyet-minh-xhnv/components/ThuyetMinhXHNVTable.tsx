...
import { IResult, IThuyetMinhXHNV } from '@/models';
import { TDTable } from '@/app/components';
import { useThuyetMinhXHNVTable } from './useThuyetMinhXHNVTable';
import { ThuyetMinhXHNVDetailModal } from './ThuyetMinhXHNVDetailModal';
import { toViewDateString } from '@/utils/utils';
import { PHUONG_THUC_KHOAN } from '@/data';

interface ThuyetMinhXHNVTableProps {
  searchData?: SearchData;
}

export const ThuyetMinhXHNVTable: React.FC<ThuyetMinhXHNVTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useThuyetMinhXHNVTable({ searchData });

  const handleAction = useCallback(
    async (type: string, record: IThuyetMinhXHNV): Promise<void> => {
      try {
        switch (type) {
          case 'detail':
            ...
            break;
          case 'delete':
            const response = await requestDELETE<IResult<boolean>>(`ThuyetMinhXHNVs/${record.id}`);
  ...
  const columns: TableProps<IThuyetMinhXHNV>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
  ...
  return (
    <>
      ...
      {modalVisible ? <ThuyetMinhXHNVDetailModal totalCount={totalCount} /> : <></>}
    </>
  );
};
