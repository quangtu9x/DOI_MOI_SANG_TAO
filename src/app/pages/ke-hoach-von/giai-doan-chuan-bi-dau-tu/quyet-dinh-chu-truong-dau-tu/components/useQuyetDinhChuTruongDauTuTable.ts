...
interface UseQuyetDinhChuTruongDauTuTableProps {
  searchData?: SearchData;
  initialPageSize?: number;
}

export const useQuyetDinhChuTruongDauTuTable = ({
  searchData,
  initialPageSize = 50,
}: UseQuyetDinhChuTruongDauTuTableProps) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const searchRequest: ISearchProjectDecisionRequest = {
        ...
        type: DecisionType.InvestmentPolicy,
        ...searchData,
      };
      ...
      if (response) {
        ...
        // Fetch thêm thông tin project cho từng record
