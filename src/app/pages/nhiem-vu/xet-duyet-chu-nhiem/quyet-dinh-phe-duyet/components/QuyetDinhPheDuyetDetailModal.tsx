...
import { AutoComplete, Avatar, DatePicker, Form, Input, Space, Spin } from 'antd';
import { Modal, Button } from 'react-bootstrap';

import * as actionsModal from '@/redux/modal/Actions';
import * as actionsGlobal from '@/redux/global/Actions';
import { RootState, AppDispatch } from '@/redux/Store';
import { IQuyetDinhPheDuyet, IPaginationResponse, IResult, IChuyenGia, IUserDetails, IBienBanMoHoSo, IChiTietMoHoSo, LoaiChiTietMoHoSo } from '@/models';
...
export const QuyetDinhPheDuyetDetailModal = props => {
  const dispatch: AppDispatch = useDispatch();
  const { totalCount } = props
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IQuyetDinhPheDuyet | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IQuyetDinhPheDuyet>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IQuyetDinhPheDuyet>>(`QuyetDinhPheDuyets/${id}`);
        const _data = response?.data?.data ?? null;
        if (_data) {
          setDinhKem(handleImage(_data?.dinhKem ?? ''));

          const chiTietQuyetDinhs = _data?.chiTietQuyetDinhs?.map(x =>
            ({ ...x, nhiemVu: x.nhiemVuId ? { label: x.nhiemVuTen, value: x.nhiemVuId } : null })
          ) ?? [];

          _data.toTrinh = _data.toTrinhId ? {
            value: _data.toTrinhId,
            label: _data.toTrinhTen,
          } : null;

          form.setFieldsValue({
            ..._data,
            chiTietQuyetDinhs: chiTietQuyetDinhs,
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const formData: IQuyetDinhPheDuyet = {
        ...values,
        ...(id && { id }),
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`QuyetDinhPheDuyets/${id}`, formData)
        : await requestPOST<IResult<string>>(`QuyetDinhPheDuyets`, formData);
            ...
            <Form<IQuyetDinhPheDuyet>
              ...
              <div className="row">
                ...
                <div className="col-xl-4 col-lg-4">
                  <Form.Item label="Tờ trình" name="toTrinh" rules={[{ required: true, message: 'Không được để trống!' }]}>
                    <TDSelect
                      ...
                      fetchOptions={async keyword => {
                        const res = await requestPOST<IPaginationResponse<any[]>>(`totrinhPheDuyets/search`, {
                          pageNumber: 1,
                          pageSize: 1000,
                          keyword: keyword,
                        });
                      ...
                      onChange={(value, current: any) => {
                        if (value) {
                          form.setFieldValue('toTrinhId', current?.id);
                        } else {
                          form.setFieldValue('toTrinhId', null);
                        }
                      }}
                    />
                  </Form.Item>
              ...
              <Form.List name="chiTietQuyetDinhs">
                {(fields, { add, remove }) => (
                  <>
                              ...
                              <td style={{ width: '30%', maxWidth: '30%' }}>
                                ...
                                >
                                  <TDSelect
                                    ...
                                    onChange={(value, current: any) => {
                                      if (value) {
                                        form.setFieldValue(['chiTietQuyetDinhs', name, 'nhiemVuId'], current?.id);
                                        fetchChiTietMoHoSos(current?.id, name);
                                      } else {
                                        form.setFieldValue(['chiTietQuyetDinhs', name, 'nhiemVuId'], null);
                                        setChiTietMoHoSosMap(prev => {
                                          const newMap = { ...prev };
                                          delete newMap[name];
                                          return newMap;
