...
import { IThuyetMinhKHCN, IPaginationResponse, IResult, PhuongThucKhoan, LoaiNhiemVu, IUserDetails } from '@/models';
...
export const ThuyetMinhKHCNDetailModal = props => {
  const dispatch: AppDispatch = useDispatch();
  const { totalCount } = props
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IThuyetMinhKHCN | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IThuyetMinhKHCN>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IThuyetMinhKHCN>>(`ThuyetMinhKHCNs/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const formData: IThuyetMinhKHCN = {
        ...values,
        ...(id && { id }),
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`ThuyetMinhKHCNs/${id}`, formData)
        : await requestPOST<IResult<string>>(`ThuyetMinhKHCNs`, formData);
          ...
          {!isLoading && (
            <Form<IThuyetMinhKHCN>
              ...
              <div className="row">
                ...
                <div className="col-xl-6 col-lg-6">
                  <Form.Item label="Phương án kỹ thuật" name="phuongAnKyThuat">
                    <Input.TextArea rows={2} placeholder="" />
                  </Form.Item>
                </div>
                <div className="col-xl-6 col-lg-6">
                  <Form.Item label="Kế hoạch ứng dụng" name="keHoachUngDung">
                    <Input.TextArea rows={2} placeholder="" />
                  </Form.Item>
                </div>
                <div className="col-xl-4 col-lg-4">
