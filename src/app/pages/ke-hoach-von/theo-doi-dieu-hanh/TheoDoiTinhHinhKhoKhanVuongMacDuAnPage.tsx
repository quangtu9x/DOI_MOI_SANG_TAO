...
import { ProjectDifficultyTable } from './components/ProjectDifficultyTable';
import { ProjectDifficultyDetailModal } from './components/ProjectDifficultyDetailModal';
import { ProjectDifficultyColumnConfigModal } from './components/ProjectDifficultyColumnConfigModal';
import { ProjectDifficultyFilterSection } from './components/ProjectDifficultyFilterSection';
import {
  exportProjectDifficultiesToExcel,
  exportProjectDifficultiesToPDF,
} from './components/useProjectDifficultyExport';
...
export const TheoDoiTinhHinhKhoKhanVuongMacDuAnPage = () => {
  ...
  const handleExport = async (format: 'excel' | 'pdf'): Promise<void> => {
    try {
      ...
      let allDifficulties: IProjectDifficulty[] = [];
      let currentPage = 1;
      ...
      while (hasMore) {
        const searchRequest: ISearchProjectDifficultyRequest = {
          ...
          category: ProjectDifficultyCategory.Difficulty, // category = 0 (Khó khăn, vướng mắc)
        ...
        if (response?.data && response.data.length > 0) {
          allDifficulties = [...allDifficulties, ...response.data];

          // Kiểm tra xem còn dữ liệu không
          const totalCount = response.totalCount || 0;
          if (allDifficulties.length >= totalCount || response.data.length < pageSize) {
            hasMore = false;
          } else {
            currentPage++;
          }
      ...
      if (allDifficulties.length > 0) {
        if (format === 'excel') {
          await exportProjectDifficultiesToExcel(allDifficulties, searchData);
          toast.success(`Xuất file Excel thành công!`);
        } else {
          await exportProjectDifficultiesToPDF(allDifficulties, searchData);
        ...
        <div className="card card-xl-stretch mb-xl-9">
          <div className="px-3 py-3 border-bottom border-secondary border-bottom-solid">
            <h3 className="card-title fw-bold text-header-td fs-4 mb-0">
              {'Theo dõi tình hình khó khăn, vướng mắc của dự án'}
            </h3>
          </div>
            ...
            <div className="card-toolbar d-flex align-items-center flex-grow-1">
              ...
              <div className="btn-group me-2 position-relative" ref={exportMenuRef}>
                <button
                  ...
                  title="Xuất file danh sách khó khăn, vướng mắc"
                >
                  <span>
                    <i
          ...
          {/* Filter Section */}
          <ProjectDifficultyFilterSection
          ...
          {/* Table */}
          <ProjectDifficultyTable searchData={searchData || searchDataRedux} />
        </div>

        {/* Modals */}
        <ProjectDifficultyDetailModal />
        <ProjectDifficultyColumnConfigModal />
      </Content>
    </>
  );
};
