...
interface UseBaoCaoNghienCuuKhaThiTableProps {
  searchData?: SearchData;
  projectProcessExecutionId?: string;
  initialPageSize?: number;
}

export const useBaoCaoNghienCuuKhaThiTable = ({
  searchData,
  projectProcessExecutionId,
  initialPageSize = 50,
}: UseBaoCaoNghienCuuKhaThiTableProps) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const searchRequest: ISearchProjectProcessStepExecutionRequest = {
        ...
        stepType: StepType.FeasibilityReport, // Filter theo báo cáo nghiên cứu khả thi
        ...
        if (responseData && responseData.length > 0) {
          ...
          if (uniqueStepIds.length > 0) {
            try {
              ...
              console.error('[useBaoCaoNghienCuuKhaThiTable] Error fetching steps:', error);
            }
          }
          ...
          if (uniqueUserIds.length > 0) {
            try {
              ...
              console.error('[useBaoCaoNghienCuuKhaThiTable] Error fetching users:', error);
            }
          }
          ...
          const enrichedData = responseData.map((item) => {
            ...
            if (item.projectProcessStepId) {
              ...
              if (step) {
                ...
                console.warn(`[useBaoCaoNghienCuuKhaThiTable] Step not found for ID: ${item.projectProcessStepId}`);
              }
            }
            ...
            if (item.assignedUserId) {
              ...
              if (user) {
                ...
                console.warn(`[useBaoCaoNghienCuuKhaThiTable] User not found for ID: ${item.assignedUserId}`);
              }
            }
