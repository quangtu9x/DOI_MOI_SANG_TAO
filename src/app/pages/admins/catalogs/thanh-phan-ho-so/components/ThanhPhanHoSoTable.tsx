...
import { ThanhPhanHoSoDetailModal } from './ThanhPhanHoSoDetailModal';
import { useThanhPhanHoSoTable } from './useThanhPhanHoSoTable';

interface CategoryTableProps {
  searchData?: SearchData;
}

export const ThanhPhanHoSoTable: React.FC<CategoryTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize, categoryGroupId } = useThanhPhanHoSoTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <ThanhPhanHoSoDetailModal totalCount={totalCount} categoryGroupId={categoryGroupId} /> : <></>}
    </>
  );
};
