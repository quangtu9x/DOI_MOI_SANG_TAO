...
import { useProjectAcceptanceTable } from './useProjectAcceptanceTable';
...
interface ProjectAcceptanceTableProps {
...
const getStatusLabel = (status?: ProjectStatus): string => {
  switch (status) {
    ...
    case ProjectStatus.Cancelled:
      return 'Hủy bỏ';
    default:
      // Nếu status = 3 (Testing) - không có trong enum hiện tại
      if (status === 3) {
...
export const ProjectAcceptanceTable: React.FC<ProjectAcceptanceTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } =
    useProjectAcceptanceTable({ searchData });
  ...
  const handleAction = async (type: string, record: IProject): Promise<void> => {
    try {
      switch (type) {
        case 'view':
