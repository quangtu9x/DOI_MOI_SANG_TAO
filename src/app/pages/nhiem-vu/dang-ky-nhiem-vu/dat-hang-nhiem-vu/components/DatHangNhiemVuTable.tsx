...
import { IResult, IDatHangNhiemVu } from '@/models';
import { TDTable } from '@/app/components';
import { useDatHangNhiemVuTable } from './useDatHangNhiemVuTable';
import { DatHangNhiemVuDetailModal } from './DatHangNhiemVuDetailModal';
import { formatNumber, toViewDate, toViewDateString } from '@/utils/utils';
import { TRANG_THAI_DAT_HANG } from '@/data';

interface DatHangNhiemVuTableProps {
  searchData?: SearchData;
}

export const DatHangNhiemVuTable: React.FC<DatHangNhiemVuTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useDatHangNhiemVuTable({ searchData });
      ...
      try {
        switch (type) {
          case 'detail':
            ...
            break;
          case 'delete':
            const response = await requestDELETE<IResult<boolean>>(`DatHangNhiemVus/${record.id}`);
