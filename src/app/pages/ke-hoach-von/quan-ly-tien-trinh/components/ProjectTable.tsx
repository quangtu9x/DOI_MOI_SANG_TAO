...
const STORAGE_KEY = 'project_column_config';
...
export const ProjectTable: React.FC<ProjectTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } =
    useProjectTable({ searchData });
  ...
  const allColumns: TableProps<IProject>['columns'] = [
    ...
    },
    {
      ...
      key: 'description',
      width: 200,
      render: (value) => (value ? <div style={{ maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={value}>{value}</div> : '-'),
    },
    {
      title: 'Mục tiêu',
