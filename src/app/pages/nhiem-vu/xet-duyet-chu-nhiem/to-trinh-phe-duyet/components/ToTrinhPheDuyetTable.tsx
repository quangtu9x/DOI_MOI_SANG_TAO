...
import { IResult, IToTrinhPheDuyet } from '@/models';
import { TDTable } from '@/app/components';
import { useToTrinhPheDuyetTable } from './useToTrinhPheDuyetTable';
import { ToTrinhPheDuyetDetailModal } from './ToTrinhPheDuyetDetailModal';
import { toViewDateString } from '@/utils/utils';

interface ToTrinhPheDuyetTableProps {
  searchData?: SearchData;
}

export const ToTrinhPheDuyetTable: React.FC<ToTrinhPheDuyetTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const modalVisibleCapMot = useSelector((state: RootState) => state.modal.modalVisibleCapMot);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useToTrinhPheDuyetTable({ searchData });

  const handleAction = useCallback(
    async (type: string, record: IToTrinhPheDuyet): Promise<void> => {
      try {
        switch (type) {
          case 'ket-qua':
          ...
          case 'detail':
            ...
            break;
          case 'delete':
            const response = await requestDELETE<IResult<boolean>>(`ToTrinhPheDuyets/${record.id}`);
  ...
  const columns: TableProps<IToTrinhPheDuyet>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
  ...
  return (
    <>
      ...
      {modalVisible ? <ToTrinhPheDuyetDetailModal totalCount={totalCount} /> : <></>}
    </>
  );
};
