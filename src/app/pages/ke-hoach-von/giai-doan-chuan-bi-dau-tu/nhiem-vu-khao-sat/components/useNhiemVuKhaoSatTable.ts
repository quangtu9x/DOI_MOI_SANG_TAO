...
interface UseNhiemVuKhaoSatTableProps {
  searchData?: SearchData;
  projectProcessExecutionId?: string;
  initialPageSize?: number;
}

export const useNhiemVuKhaoSatTable = ({
  searchData,
  projectProcessExecutionId,
  initialPageSize = 50,
}: UseNhiemVuKhaoSatTableProps) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const searchRequest: ISearchProjectProcessStepExecutionRequest = {
        ...
        stepType: StepType.SurveyTask, // Filter theo nhiệm vụ khảo sát
        ...
        if (responseData && responseData.length > 0) {
          ...
          if (uniqueStepIds.length > 0) {
            try {
              ...
              console.error('[useNhiemVuKhaoSatTable] Error fetching steps:', error);
            }
          }
          ...
          if (uniqueUserIds.length > 0) {
            try {
              ...
              console.error('[useNhiemVuKhaoSatTable] Error fetching users:', error);
            }
          }
          ...
          const enrichedData = responseData.map((item) => {
            ...
            if (item.projectProcessStepId) {
              ...
              if (step) {
                ...
                console.warn(`[useNhiemVuKhaoSatTable] Step not found for ID: ${item.projectProcessStepId}`);
              }
            }
            ...
            if (item.assignedUserId) {
              ...
              if (user) {
                ...
                console.warn(`[useNhiemVuKhaoSatTable] User not found for ID: ${item.assignedUserId}`);
              }
            }
