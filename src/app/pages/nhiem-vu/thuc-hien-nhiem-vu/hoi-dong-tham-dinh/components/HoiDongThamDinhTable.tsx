...
import { IResult, IHoiDongThamDinh } from '@/models';
import { TDTable } from '@/app/components';
import { useHoiDongThamDinhTable } from './useHoiDongThamDinhTable';
import { HoiDongThamDinhDetailModal } from './HoiDongThamDinhDetailModal';
import { toViewDateString } from '@/utils/utils';

interface HoiDongThamDinhTableProps {
  searchData?: SearchData;
}

export const HoiDongThamDinhTable: React.FC<HoiDongThamDinhTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const modalVisibleCapMot = useSelector((state: RootState) => state.modal.modalVisibleCapMot);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useHoiDongThamDinhTable({ searchData });

  const handleAction = useCallback(
    async (type: string, record: IHoiDongThamDinh): Promise<void> => {
      try {
        switch (type) {
          case 'ket-qua':
          ...
          case 'detail':
            ...
            break;
          case 'delete':
            const response = await requestDELETE<IResult<boolean>>(`HoiDongThamDinhs/${record.id}`);
  ...
  const columns: TableProps<IHoiDongThamDinh>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
  ...
  return (
    <>
      ...
      {modalVisible ? <HoiDongThamDinhDetailModal totalCount={totalCount} /> : <></>}
    </>
  );
};
