...
interface UseNghiemThuBanGiaoSanPhamTableProps {
  searchData?: SearchData;
  projectProcessExecutionId?: string;
  initialPageSize?: number;
}

export const useNghiemThuBanGiaoSanPhamTable = ({
  searchData,
  projectProcessExecutionId,
  initialPageSize = 50,
}: UseNghiemThuBanGiaoSanPhamTableProps) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const searchRequest: ISearchProjectProcessStepExecutionRequest = {
        ...
        stepType: StepType.AcceptanceHandover, // Filter theo nghiệm thu, bàn giao sản phẩm
        ...
        if (responseData && responseData.length > 0) {
          ...
          if (uniqueStepIds.length > 0) {
            try {
              ...
              console.error('[useNghiemThuBanGiaoSanPhamTable] Error fetching steps:', error);
            }
          }
          ...
          if (uniqueUserIds.length > 0) {
            try {
              ...
              console.error('[useNghiemThuBanGiaoSanPhamTable] Error fetching users:', error);
            }
          }
          ...
          const enrichedData = responseData.map((item) => {
            ...
            if (item.projectProcessStepId) {
              ...
              if (step) {
                ...
                console.warn(`[useNghiemThuBanGiaoSanPhamTable] Step not found for ID: ${item.projectProcessStepId}`);
              }
            }
            ...
            if (item.assignedUserId) {
              ...
              if (user) {
                ...
                console.warn(`[useNghiemThuBanGiaoSanPhamTable] User not found for ID: ${item.assignedUserId}`);
              }
            }
