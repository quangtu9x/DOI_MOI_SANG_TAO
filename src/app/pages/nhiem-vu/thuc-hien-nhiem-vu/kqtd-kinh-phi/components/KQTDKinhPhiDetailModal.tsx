...
import { IBienBanThamDinh, IPaginationResponse, IResult, IKeHoachThucHien, IKetQuaThamDinhKeHoach, IHoSoThamDinh, IKhoanMucKinhPhi, IKetQuaThamDinhKinhPhi, IBienBanMoHoSo, IChiTietMoHoSo, LoaiChiTietMoHoSo } from '@/models';
import { calcThanhTien, calcThanhTienDuocDuyet, formatName, getThumbnailUrl, handleFiles, handleImage, toSaveDate, toViewDate } from '@/utils/utils';
...
export const KQTDKinhPhiDetailModal = props => {
    ...
    const fetchData = async (): Promise<void> => {
      try {
        ...
        if (_data) {
          ...
          const ketQuaThamDinhKinhPhis = _data.ketQuaThamDinhKinhPhis?.map((item: IKetQuaThamDinhKinhPhi) => ({
            ...item,
            khoanMucKinhPhi: item.khoanMucKinhPhiId ? { value: item.khoanMucKinhPhiId, label: item.khoanMucKinhPhiTen } : null,
          })) ?? [];


          form.setFieldsValue({
            ..._data,
            ketQuaThamDinhKinhPhis: ketQuaThamDinhKinhPhis,
          });
        }
      } catch (error) {
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const formData: IBienBanThamDinh = {
        ...
        ketQuaThamDinhKinhPhis: values.ketQuaThamDinhKinhPhis || [],
      };

      const response = id
        ? await requestPUT<IResult<string>>(`BienBanThamDinhs/${id}/kinh-phi`, formData)
        : await requestPOST<IResult<string>>(`BienBanThamDinhs/kinh-phi`, formData);
  ...
  //         if (hoSoThamDinh) {
  //           setHoSoThamDinh(hoSoThamDinh);
  //           const khoanMucKinhPhis = (hoSoThamDinh?.khoanMucKinhPhis || []).map((item: IKhoanMucKinhPhi, index: number) => {
  //             return {
  //               ...item,
  ...
  //             hoSoThamDinhId: hoSoThamDinh.id,
  //           })
  //         }
  //       } else {
  //         form.setFieldsValue({
  ...
  //     }
  //   }
  //   if (objectSelected?.nhiemVuId && objectSelected?.chuNhiemId) {
  //     fetchData();
  //   }
  // }, [objectSelected]);
        ...
        <Spin spinning={isLoading}>
          {!isLoading && (
            <Form<any>
              ...
              {/* <HeaderTitle title={"Dự toán kinh phí đề xuất"} />
              ...
              <Form.List name="ketQuaThamDinhKinhPhis">
                {(fields, { add, remove }) => (
                  <>
                    <div className="table-responsive">
                      <table className="table table-bordered">
                        <thead className="table-light">
                          <tr>
                            <th className="text-center" style={{ width: '50px' }}>TT</th>
                            <th className="text-center" >Nội dung chi đã duyệt</th>
                            <th className="text-center" style={{ width: '15%' }}>Số lượng đã duyệt</th>
                            <th className="text-center" style={{ width: '15%' }}>Đơn giá đã duyệt</th>
                            <th className="text-center" style={{ width: '15%' }}>Thành tiền đã duyệt</th>
                              ...
                              <td>
                                ...
                                >
                                  <TDSelect
                                    ...
                                    fetchOptions={async keyword => {
                                      if (!true) return [];
                                      const res = await requestPOST<IPaginationResponse<any[]>>(`hosothamdinhs/search/kinh-phi`, {
                                        pageNumber: 1,
                                        pageSize: 1000,
                                        keyword: keyword,
                                        hoSoThamDinhId: null,
                                    ...
                                    onChange={(value, current: any) => {
                                      if (value) {
                                        form.setFieldValue(['ketQuaThamDinhKinhPhis', name, 'khoanMucKinhPhiId'], current?.id);
                                      } else {
                                        form.setFieldValue(['ketQuaThamDinhKinhPhis', name, 'khoanMucKinhPhiId'], null);
                                      }
                                    }}
                                  />
                                </Form.Item>
                              ...
                              <td>
                                ...
                                >
                                  <InputNumber<number>
                                    ...
                                    onChange={() => calcThanhTienDuocDuyet(form, 'ketQuaThamDinhKinhPhis', name)}
                                  />
                                </Form.Item>
                              </td>
                              <td>
