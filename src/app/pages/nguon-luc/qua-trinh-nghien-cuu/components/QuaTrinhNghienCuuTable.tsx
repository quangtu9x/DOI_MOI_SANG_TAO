...
import { IResult, IChuyenGiaNghienCuu } from '@/models';
import { TDTable } from '@/app/components';
import { useQuaTrinhNghienCuuTable } from './useQuaTrinhNghienCuuTable';
import { QuaTrinhNghienCuuDetailModal } from './QuaTrinhNghienCuuDetailModal';
...
export const QuaTrinhNghienCuuTable: React.FC<DataTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useQuaTrinhNghienCuuTable({ searchData });

  const handleAction = useCallback(
    async (type: string, record: IChuyenGiaNghienCuu): Promise<void> => {
      try {
        switch (type) {
          case 'detail':
            ...
            break;
          case 'delete':
            const response = await requestDELETE<IResult<boolean>>(`chuyengias/${record.id}/nghiencuu`);
  ...
  const columns: TableProps<IChuyenGiaNghienCuu>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
  ...
  return (
    <>
      ...
      {modalVisible ? <QuaTrinhNghienCuuDetailModal /> : <></>}
    </>
  );
};
