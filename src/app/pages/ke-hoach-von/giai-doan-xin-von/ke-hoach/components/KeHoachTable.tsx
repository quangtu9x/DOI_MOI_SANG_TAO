...
import { IResult, IKeHoach } from '@/models';
import { TDTable } from '@/app/components';
import { useKeHoachTable } from './useKeHoachTable';
import { KeHoachDetailModal } from './KeHoachDetailModal';
import { LOAI_BANGS, TRANG_THAIS } from '@/data';
import { formatNumber } from '@/utils/utils';

interface KeHoachTableProps {
  searchData?: SearchData;
}

export const KeHoachTable: React.FC<KeHoachTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useKeHoachTable({ searchData });
        ...
        return (
          <div>
            <a
              ...
              title="Xem chi tiết"
              onClick={() => {
                handleAction(`detail`, { ...record, readOnly: false });
              }}
            >
