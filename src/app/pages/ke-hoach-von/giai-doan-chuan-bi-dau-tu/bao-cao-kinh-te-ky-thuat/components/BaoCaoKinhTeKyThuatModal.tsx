...
export const BaoCaoKinhTeKyThuatModal = () => {
            ...
            if (stepExecution.projectProcessStepId) {
              try {
                const stepRes = await searchProjectProcessSteps({
                  pageNumber: 1,
                  pageSize: 10000,
                  stepType: StepType.TechnicalEconomicReport,
                });
    ...
    >
      <Modal.Header className="bg-primary px-4 py-3">
        <Modal.Title className="text-white">
          {dataModal?.readOnly ? 'Chi tiết' : id ? 'Chỉnh sửa' : 'Tạo mới'} báo cáo kinh tế kỹ thuật
        </Modal.Title>
        <button
          type="button"
                  ...
                  {(dataModal.stepName || dataModal.projectProcessStepName) && (
                    <div className="col-xl-6 col-lg-6">
                      <Form.Item label="Tên bước quy trình">
                        <Input value={dataModal.stepName || dataModal.projectProcessStepName} disabled={dataModal?.readOnly} />
                      </Form.Item>
                    </div>
                  )}
                  {(dataModal.stepCode || dataModal.projectProcessStepCode) && (
                    <div className="col-xl-6 col-lg-6">
                      <Form.Item label="Mã bước quy trình">
                        <Input value={dataModal.stepCode || dataModal.projectProcessStepCode} disabled={dataModal?.readOnly} />
                      </Form.Item>
                    </div>
                  )}
                  {dataModal.stepOrder !== undefined && (
                    <div className="col-xl-6 col-lg-6">
                      <Form.Item label="Thứ tự bước">
                        <Input value={dataModal.stepOrder} disabled={dataModal?.readOnly} />
                      </Form.Item>
                    </div>
                  )}
                  {dataModal.stepTypeName && (
                    <div className="col-xl-6 col-lg-6">
                      <Form.Item label="Loại bước thực hiện">
                        <Input value={dataModal.stepTypeName} disabled={dataModal?.readOnly} />
                      </Form.Item>
                    </div>
                  )}
                  {dataModal.phaseName && (
                    <div className="col-xl-6 col-lg-6">
                      <Form.Item label="Giai đoạn dự án">
                        <Input value={dataModal.phaseName} disabled={dataModal?.readOnly} />
                      </Form.Item>
                    </div>
                  )}
                </>
              ...
              {id && dataModal?.assignedByName && (
                <div className="col-xl-6 col-lg-6">
                  <Form.Item label="Tên người phân công">
                    <Input value={dataModal.assignedByName} disabled={dataModal?.readOnly} />
                  </Form.Item>
                </div>
              )}
              <div className="col-xl-6 col-lg-6">
