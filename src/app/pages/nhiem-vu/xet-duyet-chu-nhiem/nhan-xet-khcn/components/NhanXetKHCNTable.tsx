...
import { useNhanXetKHCNTable } from './useNhanXetKHCNTable';
import { NhanXetKHCNDetailModal } from './NhanXetKHCNDetailModal';
import { toViewDateString } from '@/utils/utils';

interface NhanXetKHCNTableProps {
  searchData?: SearchData;
}

export const NhanXetKHCNTable: React.FC<NhanXetKHCNTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const modalVisibleCapMot = useSelector((state: RootState) => state.modal.modalVisibleCapMot);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useNhanXetKHCNTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <NhanXetKHCNDetailModal totalCount={totalCount} /> : <></>}
    </>
  );
};
