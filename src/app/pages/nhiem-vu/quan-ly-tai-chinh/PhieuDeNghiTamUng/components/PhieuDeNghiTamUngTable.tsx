...
import { IPhieuDeNghiTamUng, IResult, TrangThaiPhieu } from '@/models';
import { TDTable } from '@/app/components';
import { usePhieuDeNghiTamUngTable } from './usePhieuDeNghiTamUngTable';
import { formatNumber, toViewDateString } from '@/utils/utils';

interface PhieuDeNghiTamUngTableProps {
  searchData?: SearchData;
}

export const PhieuDeNghiTamUngTable: React.FC<PhieuDeNghiTamUngTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } =
    usePhieuDeNghiTamUngTable({ searchData });

  const handleAction = useCallback(
    async (type: string, record: IPhieuDeNghiTamUng): Promise<void> => {
      try {
        switch (type) {
          case 'detail':
            ...
            break;
          case 'disbursement':
            dispatch(actionsModal.setDataModalCapMot({ phieuDeNghiTamUngId: record.id }));
            dispatch(actionsModal.setModalVisibleCapMot(true));
            break;
          case 'delete':
            const response = await requestDELETE<IResult<boolean>>(`PhieuDeNghiTamUngs/${record.id}`);
  ...
  const columns: TableProps<IPhieuDeNghiTamUng>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
      ...
      render: (text, record) => {
        return (
          <a
            className="fw-bold"
            onClick={() => {
              dispatch(actionsModal.setDataModal({ ...record, readOnly: true } as any));
              dispatch(actionsModal.setModalVisible(true));
            }}
          >
            {text}
