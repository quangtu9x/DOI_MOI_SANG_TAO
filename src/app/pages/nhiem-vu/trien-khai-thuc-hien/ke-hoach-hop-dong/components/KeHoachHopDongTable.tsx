...
import { IResult, IKeHoachHopDong } from '@/models';
import { TDTable } from '@/app/components';
import { useKeHoachHopDongTable } from './useKeHoachHopDongTable';
import { KeHoachHopDongDetailModal } from './KeHoachHopDongDetailModal';
import { formatNumber, toViewDateString } from '@/utils/utils';
import { TU_DANH_GIA } from '@/data';

interface KeHoachHopDongTableProps {
  searchData?: SearchData;
}

export const KeHoachHopDongTable: React.FC<KeHoachHopDongTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useKeHoachHopDongTable({ searchData });

  const handleAction = useCallback(
    async (type: string, record: IKeHoachHopDong): Promise<void> => {
      try {
        switch (type) {
          case 'detail':
            ...
            break;
          case 'delete':
            const response = await requestDELETE<IResult<boolean>>(`KeHoachHopDongs/${record.id}`);
  ...
  const columns: TableProps<IKeHoachHopDong>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
    ...
    },
    {
      ...
      dataIndex: 'nguoiPhuTrachHoTen',
      key: 'nguoiPhuTrachHoTen',
      className: 'text-center',
      width: '12%',
      render: data => formatNumber(data),
    },
  ...
  return (
    <>
      ...
      {modalVisible ? <KeHoachHopDongDetailModal totalCount={totalCount} /> : <></>}
    </>
  );
};
