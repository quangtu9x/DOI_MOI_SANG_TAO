...
import { useHDNTCoSoTable } from './useHDNTCoSoTable';
import { HDNTCoSoDetailModal } from './HDNTCoSoDetailModal';
import { toViewDateString } from '@/utils/utils';

interface HDNTCoSoTableProps {
  searchData?: SearchData;
  readOnly?: boolean;
}

export const HDNTCoSoTable: React.FC<HDNTCoSoTableProps> = ({ searchData, readOnly }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const modalVisibleCapMot = useSelector((state: RootState) => state.modal.modalVisibleCapMot);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useHDNTCoSoTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <HDNTCoSoDetailModal totalCount={totalCount} /> : <></>}
    </>
  );
};
