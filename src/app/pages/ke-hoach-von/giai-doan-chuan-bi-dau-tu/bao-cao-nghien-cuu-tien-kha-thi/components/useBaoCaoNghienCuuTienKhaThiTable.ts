...
interface UseBaoCaoNghienCuuTienKhaThiTableProps {
  searchData?: SearchData;
  projectProcessExecutionId?: string;
  initialPageSize?: number;
}

export const useBaoCaoNghienCuuTienKhaThiTable = ({
  searchData,
  projectProcessExecutionId,
  initialPageSize = 50,
}: UseBaoCaoNghienCuuTienKhaThiTableProps) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const searchRequest: ISearchProjectProcessStepExecutionRequest = {
        ...
        stepType: StepType.PreFeasibilityReport, // Filter theo báo cáo nghiên cứu tiền khả thi
        ...
        if (responseData && responseData.length > 0) {
          ...
          if (uniqueStepIds.length > 0) {
            try {
              ...
              console.error('[useBaoCaoNghienCuuTienKhaThiTable] Error fetching steps:', error);
            }
          }
          ...
          if (uniqueUserIds.length > 0) {
            try {
              ...
              console.error('[useBaoCaoNghienCuuTienKhaThiTable] Error fetching users:', error);
            }
          }
          ...
          const enrichedData = responseData.map((item) => {
            ...
            if (item.projectProcessStepId) {
              ...
              if (step) {
                ...
                console.warn(`[useBaoCaoNghienCuuTienKhaThiTable] Step not found for ID: ${item.projectProcessStepId}`);
              }
            }
            ...
            if (item.assignedUserId) {
              ...
              if (user) {
                ...
                console.warn(`[useBaoCaoNghienCuuTienKhaThiTable] User not found for ID: ${item.assignedUserId}`);
              }
            }
