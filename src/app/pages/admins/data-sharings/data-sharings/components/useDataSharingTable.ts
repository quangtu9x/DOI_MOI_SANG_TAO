...
import { IPaginationResponse, IDataSharing } from '@/models';
...
export const useDataSharingTable = ({ searchData, initialPageSize = 50 }: UseDataTableProps) => {
  const dispatch: AppDispatch = useDispatch();
  const random = useSelector((state: RootState) => state.global.random);

  const [data, setData] = useState<IDataSharing[]>([]);
