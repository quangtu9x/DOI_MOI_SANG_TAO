...
import { useDanhGiaXHNVTable } from './useDanhGiaXHNVTable';
import { DanhGiaXHNVDetailModal } from './DanhGiaXHNVDetailModal';
import { toViewDateString } from '@/utils/utils';

interface DanhGiaXHNVTableProps {
  searchData?: SearchData;
}

export const DanhGiaXHNVTable: React.FC<DanhGiaXHNVTableProps> = ({ searchData }) => {
  const dispatch: AppDispatch = useDispatch();
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const modalVisibleCapMot = useSelector((state: RootState) => state.modal.modalVisibleCapMot);

  const { data, loading, totalCount, currentPage, pageSize, setCurrentPage, setPageSize } = useDanhGiaXHNVTable({ searchData });
  ...
  return (
    <>
      ...
      {modalVisible ? <DanhGiaXHNVDetailModal totalCount={totalCount} /> : <></>}
    </>
  );
};
