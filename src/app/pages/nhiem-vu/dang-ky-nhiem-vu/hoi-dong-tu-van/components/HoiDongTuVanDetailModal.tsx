...
import { IHoiDongTuVan, IPaginationResponse, IResult, IUserDetails } from '@/models';
...
export const HoiDongTuVanDetailModal = props => {
  const dispatch: AppDispatch = useDispatch();
  const { totalCount } = props
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IHoiDongTuVan | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IHoiDongTuVan>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IHoiDongTuVan>>(`hoidongtuvans/${id}`);
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`HoiDongTuVans/${id}`, formData)
        : await requestPOST<IResult<string>>(`HoiDongTuVans`, formData);
