...
import { IPaginationResponse, IHoSoNghiemThu, IResult, CapNghiemThu } from '@/models';
...
export const HoSoNghiemThuDetailModal: React.FC<Props> = ({ totalCount }) => {
  const dispatch: AppDispatch = useDispatch();
  const dataModal = useSelector((state: RootState) => state.modal.dataModal) as IHoSoNghiemThu | null;
  const modalVisible = useSelector((state: RootState) => state.modal.modalVisible);
  const id = dataModal?.id ?? null;

  const [form] = Form.useForm<IHoSoNghiemThu>();
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      try {
        setIsLoading(true);
        const response = await requestGET<IResult<IHoSoNghiemThu>>(`HoSoNghiemThus/${id}`);
        ...
        if (_data) {
          ...
          _data.hoiDongNghiemThu = _data.hoiDongNghiemThuId ? { label: _data.hoiDongNghiemThuTen, value: _data.hoiDongNghiemThuId } : null;
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const response = id
        ? await requestPUT<IResult<string>>(`HoSoNghiemThus/${id}`, formData)
        : await requestPOST<IResult<string>>(`HoSoNghiemThus`, formData);
    ...
    >
      ...
      <Modal.Body>
        <Spin spinning={isLoading}>
          {!isLoading && (
            <Form<IHoSoNghiemThu>
              initialValues={{ isActive: true, capNghiemThu: CapNghiemThu.ChinhThuc }}
              ...
              <div className="row">
                ...
                <div className="col-xl-8 col-lg-8">
                  <Form.Item label="Hội đồng nghiệm thu" name="hoiDongNghiemThu"
                ...
                <div className="col-xl-4 col-lg-4">
                  <Form.Item label="Ngày nộp hồ sơ" name='ngayNopHoSo'
                    initialValue={dayjs()}
                    rules={[{ required: true, message: 'Không được để trống!' }]}>
                ...
                <div className="col-xl-4 col-lg-4">
                  <Form.Item label="Báo cáo tổng kết" name="baoCaoTongKet" rules={[{ required: true, message: 'Không được để trống!' }]}>
                    <Input.TextArea rows={2} placeholder=' ' />
                  </Form.Item>
                </div>
                <div className="col-xl-4 col-lg-4">
                  <Form.Item label="Báo cáo tóm tắt" name="baoCaoTomTat">
                    <Input.TextArea rows={2} placeholder=' ' />
                  </Form.Item>
                </div>
                <div className="col-xl-4 col-lg-4">
                  <Form.Item label="Báo cáo về sản phẩm" name="baoCaoVeSanPham">
                    <Input.TextArea rows={2} placeholder=' ' />
                  </Form.Item>
                </div>
                <div className="col-xl-4 col-lg-4">
