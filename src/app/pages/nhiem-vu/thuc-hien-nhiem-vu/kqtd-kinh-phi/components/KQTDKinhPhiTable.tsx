...
import { IResult, IBienBanThamDinh } from '@/models';
import { TDTable } from '@/app/components';
import { useHoSoThamDinhTable } from './useKQTDKinhPhiTable';
import { KQTDKinhPhiDetailModal } from './KQTDKinhPhiDetailModal';
import { formatNumber, toViewDateString } from '@/utils/utils';

interface KQTDKinhPhiTableProps {
  searchData?: SearchData;
}

export const KQTDKinhPhiTable: React.FC<KQTDKinhPhiTableProps> = ({ searchData }) => {
  ...
  const handleAction = useCallback(
    async (type: string, record: IBienBanThamDinh): Promise<void> => {
      try {
        switch (type) {
          case 'ket-qua':
          ...
          case 'detail':
            ...
            break;
          case 'delete':
            const response = await requestDELETE<IResult<boolean>>(`BienBanThamDinhs/${record.id}`);
  ...
  const columns: TableProps<IBienBanThamDinh>['columns'] = [
    {
      title: 'STT',
      dataIndex: 'index',
      key: 'index',
    ...
    {
      title: 'Tổng kinh phí được duyệt (VNĐ)',
  ...
  return (
    <>
      ...
      {modalVisible ? <KQTDKinhPhiDetailModal totalCount={totalCount} /> : <></>}
    </>
  );
};